...
export function OpprettSakForm({ assetId }: Props) {
  ...
  async function send(e: React.FormEvent) {
    ...
    if (!innsender.trim()) { setError("Navn/kontakt er påkrevd"); return; }
    ...
    try {
      ...
      form.append("innsender", innsender.trim());
  ...
  return (
    <div style={s.card}>
      ...
      <form onSubmit={send} style={s.form}>
        ...
        <label style={s.label}>
          Ditt navn / kontaktinfo <span style={s.req}>*</span>
          <input
            type="text"
            value={innsender}
            onChange={(e) => setInnsender(e.target.value)}
        ...
        <div style={s.label}>
          ...
          />
          {bilde && (
            <p style={s.filnavn}>{bilde.name}</p>
          )}
        </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  avbryt: { padding: "0.55rem 1rem", fontSize: "0.9rem", color: "#6b7280", border: "1px solid #e2e8f0", borderRadius: 7, cursor: "pointer", textDecoration: "none", display: "inline-flex", alignItems: "center" },
